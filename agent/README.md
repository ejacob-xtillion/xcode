...
This directory is the **agent HTTP service** for the [xCode](../README.md) monorepo: FastAPI, LangGraph agents, MCP tool integration, and SSE streaming.

## What You Have

This package contains a **working FastAPI application** with:
...
- **Add custom tools** - Register functions in `app/engine/custom_tools.py` (`get_all_tools()`)
...
- Add Python functions in `app/engine/custom_tools.py` and expose them via `get_all_tools()`
