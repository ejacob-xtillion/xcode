...
- If the task is unclear, ambiguous, or not a valid coding request, respond
  immediately without using tools
...
Please complete the task now."""
    ...
    def _handle_event(self, event: dict, logs: list[str]) -> None:
        ...
        if event_type == "session_created":
            ...
            if tool_call_id:
                result_info.append("ID: ", style="dim cyan")
            ...
            if content:
                ...
                if len(content_str) > 1000 and not self.config.verbose:
                    ...
                    result_info.append(
                        f"... [dim]({len(content_str) - 500} more chars, "
            ...
            self.console.print(Panel(result_info, border_style=border_style, padding=(0, 1)))
    ...
    def _get_file_cache(self) -> str | None:
        """
        ...
        try:
            ...
            for directory in sorted_dirs:
                ...
                if len(files) > max_files:
                    ...
                    tree_lines.append(
                        f"  ... ({len(files_by_dir[directory]) - max_files} more files)"
                    )
                else:
                    for fname in files:
                        tree_lines.append(f"  {fname}")
    ...
    def _verify_result(self, logs: list[str]) -> tuple[bool, str | None]:
        """
