...
class ExecutionTracker:
    ...
    def check_should_stop(self) -> tuple[bool, Optional[StopReason]]:
        """
        ...
        total_tool_calls = sum(
            len(it.tool_calls) for it in self.iterations
        )
        if self.current_iteration:
    ...
    def get_stats(self) -> Dict[str, any]:
        ...
        successful_calls = sum(
            sum(1 for tc in it.tool_calls if tc.success)
            for it in self.iterations
        )
        if self.current_iteration:
            successful_calls += sum(
                1 for tc in self.current_iteration.tool_calls if tc.success
            )
